...
def mainloop() :
    from timeit import timeit
    print("loop with list" , timeit(lp1 , number = 100))
    print("loop with tuple" , timeit(tp1 , number = 100))
    x = timeit(lp1 , number = 1)
    y = timeit(tp1 , number = 1)
    print(f"list \ndifference - {y-x}" if x<y else f"tuple \ndifference - {x-y}")
...
#? From muliple results creation of tuples is faster than lists 
#? From muliple results loop through tuples is faster than lists 
